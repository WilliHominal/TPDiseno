...
(3528000000200, '28364781', 'NO_VIGENTE',  '2019-02-15', '2019-07-15', 'SEMESTRAL',   false, '2019-02-16', '60.000 - 69.999',   'DBASDA18628364781', 'UNO', 'MAA872', 555555, true,  false, false, 0.65, 5000, 116000, 115000,
		...
		(select r.valor_porcentual from riesgo_tipo_cobertura r  where r.fin_vigencia is null and r.tipo_cobertura = 'RESP_CIVIL_ROBO_O_INCENDIO_TOTAL'),
		(select r.valor_porcentual from riesgo_ciudad r  where r.fin_vigencia is null and r.id_ciudad = 5314),  120, 5314, 5400000002, 100, null, 'TODO_RIESGO_CON_FRANQUICIA'),
...
(3528000000300, '93842993', 'NO_VIGENTE',  '2018-02-11', '2018-08-11', 'SEMESTRAL',   false, '2018-02-12', '60.000 - 69.999',   'F8CV120123345', 'UNO', 'MAA872', 555555, true,  false, false, 0.65, 5000, 116000, 115000,
		(select r.valor_porcentual from riesgo_modelo r, anio_modelo an where r.fin_vigencia is null and r.id_modelo=an.id_modelo and an.id = 120),
		(select r.valor_porcentual from riesgo_tipo_cobertura r  where r.fin_vigencia is null and r.tipo_cobertura = 'TODO_TOTAL'),
		(select r.valor_porcentual from riesgo_ciudad r  where r.fin_vigencia is null and r.id_ciudad = 5314),  120, 5314, 5400000003, 100, null, 'TODO_RIESGO_CON_FRANQUICIA');




INSERT INTO cuota (id_cuota, estado, monto, ultimo_dia_pago, numero_poliza, idx) VALUES
(NEXTVAL('id_cuota_seq'), 'IMPAGO', 23424, '2019-11-05', 3528000000105, 0);